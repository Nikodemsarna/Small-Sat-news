...
def test_no_keys_means_no_summarizer(monkeypatch):
    s = Settings.from_env()
    assert s.provider == "none"
    assert s.has_summarizer is False
...
def test_gemini_autodetected(monkeypatch):
    ...
    assert s.has_summarizer is True
    assert s.resolved_model == "gemini-2.0-flash"
    assert s.api_key == "x"
